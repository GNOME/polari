...
var ChatView = GObject.registerClass({
    ...
    _getPixelWidthFromAvg(charCount) {
    ...
    _updateVisibleIndent() {
        ...
        do {
            if (start.is_end())
                break;
            let next = this._setNickOrEllipsisAtIter(start, this._msgNickPixels);
            start = next;
    ...
    _updateIndent() {
        ...
        this._view.left_margin = char_margin*2 + totalWidth;
    }
    ...
    _replaceWithTags(iter, iterEnd, text, tags) {
        let buffer = this._view.get_buffer();
        let offset = iter.get_offset(); // starting point in buffer
